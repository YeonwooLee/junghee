...
def timer():
    ...
    return result.split(':')
def send_user(user,msg):
    kakao_opentalk_name = user
    open_chatroom(kakao_opentalk_name)  # 채팅방 열기
    kakao_sendtext(kakao_opentalk_name, msg)  # 메시지 전송

def sendMsg(msg):
    kakao_opentalk_name = "JhinEye"
    open_chatroom(kakao_opentalk_name)  # 채팅방 열기
    kakao_sendtext(kakao_opentalk_name, msg)  # 메시지 전송

def send_admin(msg):
    kakao_opentalk_name = "aa"
    open_chatroom(kakao_opentalk_name)  # 채팅방 열기
    kakao_sendtext(kakao_opentalk_name, msg)  # 메시지 전송

if __name__ == "__main__":
    for i in range(3):
        send_user("김재구", "목탁! 연속동작 테스트")
# else:
#     set_hour = '06'
